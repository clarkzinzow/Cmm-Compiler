...
class DeclListNode extends ASTnode {
  ...
  public void typeCheck() {
    ...
    for (DeclNode node : myDecls) {
      if(node instanceof FnDeclNode) {
        if(((FnDeclNode)node).getName().equals("main")) {
          hasMain = true;
        }
      }
      node.typeCheck();
  ...
  public int setOffsets(int offset) {
    for(DeclNode node : myDecls) {
      offset = ((VarDeclNode)node).setOffset(offset);
    }
    return offset;
  }
...
class FnBodyNode extends ASTnode {
  ...
  public void nameAnalysis(SymTable symTab) {
    ...
    localSize = myStmtList.setOffsets(myDeclList.setOffsets(8));
    localSize -= 8;
...
class FnDeclNode extends DeclNode {
  ...
  public void codegen() {
    ...
    int paramsSize = ((FnSym)(myId.sym())).getParamsSize();
    int localSize = myBody.getLocalSize();
    String exitLabel = "_" + myId.name() + "_Exit";
    ((FnSym)myId.sym()).setExit(exitLabel);
    ...
    Codegen.genPush("$ra");
    Codegen.genPush("$fp");
    Codegen.generate("addu", "$fp", "$sp", paramsSize + 8);
    if(localSize != 0) {
      Codegen.generate("subu", "$fp", "$sp", localSize);
    }
    myBody.codegen(exitLabel);
    Codegen.genLabel(exitLabel, "FUNCTION EXIT");
    Codegen.generateIndexed("lw", "$ra", "$fp", -paramsSize);
    Codegen.generate("move", "$t0", "$fp");
    Codegen.generateIndexed("lw", "$fp", "$fp", -paramsSize - 4);
...
abstract class StmtNode extends ASTnode {
  ...
  public int setOffsets(int nodeOffset) {
    return nodeOffset;
  }
}
...
class PostIncStmtNode extends StmtNode {
  ...
  public void codegen(String exitLabel) {
    ...
    if(idNode.sym().isGlobal()) {
      Codegen.generate("sw", "$t0", "_" + idNode.name());
    }
    else {
      Codegen.generateIndexed("sw", "$t0", "$fp", idNode.sym().getOffset());
    }
  }
...
class ReadStmtNode extends StmtNode {
  ...
  public void codegen(String exitLabel) {
    ...
    Codegen.generate("syscall");
    if(idNode.sym().isGlobal()) {
      Codegen.generate("sw", "$v0", "_" + idNode.name());
    }
    else {
      Codegen.generateIndexed("sw", "$v0", "$fp", idNode.sym().getOffset());
    }
  }
...
class IfStmtNode extends StmtNode {
  ...
  public int setOffsets(int offset) {
    return myStmtList.setOffsets(myDeclList.setOffsets(offset));
  }
  ...
  public void codegen(String exitLabel) {
    ...
    myExp.codegen();
    Codegen.genPop("$t0");
    Codegen.generate("beq", "$t0", "0", falseLabel);
    myStmtList.codegen(exitLabel);
    Codegen.genLabel(falseLabel, "endif");
  }
...
class IfElseStmtNode extends StmtNode {
  ...
  public int setOffsets(int offset) {
    offset = myThenStmtList.setOffsets(myThenDeclList.setOffsets(offset));
    return myElseStmtList.setOffsets(myElseDeclList.setOffsets(offset));
  }
  ...
  public void codegen(String exitLabel) {
    ...
    Codegen.genLabel(falseLabel, "else block");
    myElseStmtList.codegen(exitLabel);
    Codegen.genLabel(doneLabel, "end if-else");
  }
...
class WhileStmtNode extends StmtNode {
  ...
  public void codegen(String exitLabel) {
    String conditionLabel = Codegen.nextLabel();
    String doneLabel = Codegen.nextLabel();
    Codegen.genLabel(conditionLabel, "while condition");
    myExp.codegen();
    Codegen.genPop("$t0");
    Codegen.generate("beq", "$t0", "0", doneLabel);
    myStmtList.codegen(exitLabel);
    Codegen.generate("b", conditionLabel);
    Codegen.genLabel(doneLabel, "end while");
  }
...
class CallExpNode extends ExpNode {
  ...
  public void codegen() {
    myExpList.codegen();
    Codegen.generate("jal", ((FnSym)myId.sym()).getEntry());
    Codegen.genPush("$v0");
  }
...
abstract class LogicalExpNode extends BinaryExpNode {
  ...
  public void codegen() {
    ...
    myExp1.codegen();
    Codegen.genPop("$t0");
    Codegen.generate("beq", "$t0", myOp, doneLabel);
    myExp2.codegen();
    Codegen.genPop("$t0");
...
abstract class EqualityExpNode extends BinaryExpNode {
  ...
  public void codegen() {
    ...
    Codegen.genPop("$t1");
    Codegen.genPop("$t0");
    Codegen.generate(myOp, "$t0", "$t1", trueLabel);
    Codegen.generate("li", "$t0", 0);
    Codegen.generate("b", doneLabel);
...
abstract class RelationalExpNode extends BinaryExpNode {
  ...
  public void codegen() {
    ...
    Codegen.generate(myOp, "$t0", trueLabel);
    Codegen.generate("li", "$t0", 0);
    Codegen.generate("b", doneLabel);
    Codegen.genLabel(trueLabel);
    Codegen.generate("li", "$t0", 1);
