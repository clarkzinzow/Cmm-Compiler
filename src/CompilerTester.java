...
public class CompilerTester {
	...
	/**
	 * CompilerTester constructor for client programs and testers. Note that
	...
	/**
	 * If we are directly invoking CompilerTester from the command line, this
	 ...
	 */
	private CompilerTester(String[] args){
    	//Parse arguments    	
        if (args.length < 2) {
	...
	public int process(){
		...
		if (ErrMsg.getErr()) {  
			return CompilerTester.RESULT_SYNTAX_ERROR;
		}
		
		astRoot.nameAnalysis();  // perform name analysis
		
		if (ErrMsg.getErr()) {  
			return CompilerTester.RESULT_NAME_ERROR;
		}
		
		astRoot.typeCheck();
		
		if (ErrMsg.getErr()) {  
			return CompilerTester.RESULT_TYPE_ERROR;
		}
		
		astRoot.codegen();
		return CompilerTester.RESULT_CORRECT;
	}
    ...
    public static void main(String[] args){
    	CompilerTester instance = new CompilerTester(args);
    	instance.run();
    }
}
